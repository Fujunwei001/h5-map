...
SquareOverlay.prototype.draw = function () {
  ...
  var position = this._map.pointToPixel(this._start)
...
const initMap = () => {
  if (typeof BMap !== 'undefined') {
    ...
    // 添加自定义覆盖物
    //[106.575354, 29.558923] [39.906217, 116.3912757]
    var mySquare = new SquareOverlay(
      new BMap.Point(106.575354, 29.558923),
...
const handleScroll = () => {
  ...
  if (newPage !== currentPage.value) {
    ...
    console.log('翻页', '当前页:', newPage + 1)

    // 这里可以触发地图事件
    triggerMapEvent(newPage)
  }
}
...
const playArrowAnimation = () => {
  ...
  arrowOverlay.lengthProgress = 0
  function step() {
    arrowOverlay.lengthProgress += 0.01
    if (arrowOverlay.lengthProgress > 1) arrowOverlay.lengthProgress = 1
    arrowOverlay.draw()
    if (arrowOverlay.lengthProgress < 1) requestAnimationFrame(step)
  }
  step()
}

// 触发地图事件
const triggerMapEvent = (pageIndex) => {
  console.log('翻页')
...
// 重新播放图片动画
const replayImageAnimation = () => {
  const imageEl = document.querySelector('.curve-image')
  if (imageEl) {
    // 移除动画类
    ...
    // 重新添加动画
    imageEl.style.animation = 'scaleUp 2s ease-out forwards'
  }
}
...
const handleOrientationChange = () => {
  checkOrientation()
  // 如果切换到横屏且地图未初始化，则初始化地图
  if (!isPortrait.value && !map) {
    nextTick(() => {
...
.test1 {
  ...
  height: 200px;
  /* animation: scaleUp 2s linear forwards;
  transform: rotate(var(--rotation)) scaleY(0) scaleX(0.2); */
  transform-origin: bottom;
}
@keyframes scaleUp {
  0% {
  ...
  100% {
    /* 动画结束：保留旋转变量 + 最终缩放 */
    transform: rotate(var(--rotation)) scaleY(1) scaleX(0.2);
    opacity: 1;
  }
}
